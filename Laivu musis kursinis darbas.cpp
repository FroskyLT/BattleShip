...
public:
	...
	void GameMap() {
		...
		for (int i = 0; i < 10; i++)
		{
			...
			for (int j = 0; j < 10; j++)
			{
				if (MyShips[i][j] == 0) cout << setw(2) << '.'; // musu korta
				...
				if (j == 9)
				{
					...
					for (int k = 0; k < 10; k++)
					{
						if (ShotsMap[i][k] == 0) cout << setw(2) << '.'; // saudymo korta
		...
		cout << setw(8) << " " << "Jusu korta" << setw(41) << "Suviu korta" << endl;
		cout << endl;
	}
};
...
void SetPosition::ManualSet(int sh[][il]) {
	...
	while (!a.empty()) {
		ShowMap(sh);
		cout << "Jums liko nepanaudotu laiveliu: " << endl;
		for (int i = 0; i < count; i++) cout << "X";
		cout << " " << a[count] << " vnt.\n";
		if (count > 1) cout << "iveskite koordinates pvz (a1 a" << count << ")" << endl;
		else cout << "iveskite koordinates pvz (a" << count << ")" << endl;
		...
		while (poscheck) {
			...
			if (CoordCheck(x1, y1, x2, y2, count))
			{
				...
				else { cout << "sioje vietoje negalima pastatyti laivelio\n"; }
			}
		}
...
void SetPosition::RandomSet(int sh[][il]) {
	...
	while (!a.empty()) {
		...
		while (poscheck)
		{
			...
			{
				if (x1 + count > 10) // jei reikes vertikaliai
				{
					x2 = x1;
					y2 = y1 + count - 1;
				}
				else if (y1 + count > 10) // jei reikes horizontaliai
				{
					x2 = x1 + count - 1;
					y2 = y1;
				}
...
void Shots::operator++() {
	...
	while (checkpos)
	{
		...
		if (ShotCheck(x, y))
		{
			...
			else if (EShips[y][x] == -1) { cout << "cia jau yra sudauzytas laivas\n"; }
			else { cout << "cia jau saudete\n"; }
		}
	}
}
void Shots::operator--()
...
int main() {
	...
	cout << "Atsitiktinis isdestymas - 0\nSavarankiskas isdestymas - 1\n";
	int temp; cin >> temp;
	...
	{
		if (shots.Turn()) {
			cout << "iveskite saudimo kooridnates: ";
			++shots;
			system("CLS");
			show.GameMap();
			/*show.ShowMap(Ships::EShips);*/
			if (shots.Turn()) { cout << "Opa! pataiket, davai escio\n"; }
		}
		else {
			--shots;
			system("CLS");
	...
	system("CLS");
	if (MySum == 0) cout << "Jus praleimejote :(" << endl;
	else cout << "Sveikiname, Jus laimejote!" << endl;
